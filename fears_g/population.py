...
from fears_g.utils import pharm
...
class Population:
    ...
    def __init__(self,dr_params=None,game_params=None,carrying_cap=10**6,
        ...
        if game_params is None:
            ...
            game_params.append([0.173,-0.0033]) 
        if dr_params is None:
            dr_params = []
            dr_params.append([0.13198219, 0.05040618, 1.81487953, 0.16802087]) # gmax, gmin, hc, ic50
            dr_params.append([0.1150929 , 0.09042498, 3.35519454, 0.1750947 ])
        
        self.n_genotype = len(dr_params)
        if init_counts is None:
            self.init_counts = np.ones(self.n_genotype)*10**3
        else:
    ...
    def gen_growth_rate(self,genotype_num,conc,frac):
        dr_params = self.dr_params[genotype_num]
        game_params = self.game_params[genotype_num]
        g_100 = self.hill_fn(conc,*dr_params)
        g_frac = self.fds_model(frac,g_100,*game_params)
        return g_frac
    
    def plot_unified_model(self,genotype_num=0,legend=True,conc_range=None):
        ...
        for f in np.linspace(0,1,11):
            ax.plot(conc_range,self.gen_growth_rate(genotype_num,conc_range,f),
    ...
    def gen_fitness_landscape(self,counts,conc,carrying_cap=False):
        n_genotype = len(counts)
        fitness_landscape = np.zeros(n_genotype)
        frac_list = counts/np.sum(counts)

        for i in range(n_genotype):
            fitness_landscape[i] = self.gen_growth_rate(i,conc,frac_list[i])

        if carrying_cap:
            fitness_landscape = fitness_landscape*(1-np.sum(counts)/self.carrying_cap)

        fitness_landscape[fitness_landscape<0] = 0

        return fitness_landscape
    
    def passage_cells(self,mm,counts):
        if self.passage:
            if np.mod(mm,self.passage_time) == 0:
    ...
    def abm(self,mm,counts):
        ...
        fit_land = self.gen_fitness_landscape(counts,conc,carrying_cap=True)
        # fit_land_no_cc = self.gen_fitness_landscape(counts,conc,carrying_cap=False)
        # self.fitness_timecourse[mm+1,:] = fit_land_no_cc
        ...
        counts_t = counts_t - np.random.poisson(counts*self.death_rate)
        ...
        if all(counts_t == 0):
            ...
            counts_t += np.random.poisson(counts_t*fit_land)
    ...
    def run_abm(self):
        ...
        n_genotype = len(self.game_params)

        mm = 0

        counts = np.zeros( [self.n_timestep, n_genotype] , dtype=int)
        counts[0,:] = self.init_counts
        self.fitness_timecourse[0,:] = self.gen_fitness_landscape(counts[0,:],self.drug_curve[0])
        
        while mm < self.n_timestep-1:
            self.fitness_timecourse[mm+1,:] = self.gen_fitness_landscape(counts[mm,:],self.drug_curve[mm])
            counts[mm+1] = self.abm(mm,counts[mm])
            mm+=1
    ...
    def simulate(self):
        for i in range(self.n_sims):
            ...
            if i == 0:
                ...
                counts_avg += counts
        counts_avg = counts_avg/self.n_sims
        
        self.counts_avg = counts_avg

    def plot_simulation(self,counts=None):
        if counts is None:
            if self.counts_avg is None:
                counts, mm = self.run_abm()
            else:
                counts = self.counts_avg

        fig,ax = plt.subplots()
        cmap = self.cmap

        for i in range(self.n_genotype):
            ax.plot(np.arange(self.n_timestep),counts[:,i],color=cmap(i/(self.n_genotype-1)),
                    label=self.legend_labels[i],linewidth=2)
    ...
    def plot_fitness_timecourse(self):

        fig,ax = plt.subplots()
        cmap = self.cmap

        for i in range(self.n_genotype):
            ax.plot(np.arange(self.n_timestep),self.fitness_timecourse[:,i],
                    color=cmap(i/(self.n_genotype-1)),
                    label=self.legend_labels[i],linewidth=2)
